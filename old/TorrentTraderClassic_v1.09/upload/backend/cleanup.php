...
function autoinvites($length, $minlimit, $maxlimit, $minratio, $invites)
{
    ...
    $res = DB::query("
        ...
        WHERE class > 0
            AND enabled = 'yes'
            AND downloaded >= $minlimit
            AND downloaded < $maxlimit
    ...
    if ($res) {
        while ($arr = $res->fetch()) {
            ...
            elseif ($arr['invites'] == 8 && $invites == 3)
                $invites = 2;
            DB::query("UPDATE users SET invites = invites + $invites, invitedate = NOW()
...
function docleanup()
{
	...
	do {
        ...
        if ($RATIO_WARNINGON) {
            ...
            $query = DB::query("SELECT id, downloaded, uploaded FROM users WHERE class = 0 AND donated = 0 AND downloaded > 0");
                ...
                if ($userratio <= $RATIOWARN_AMMOUNT) {
                    ...
                    if ((mysql_num_rows($check) != "0")){
                        ...
                        if ($status['warned'] == "yes"){
                            ...
                            if ($notificationdate['difference'] > $RATIOWARN_TIME) {
                                //if notification date is > $RATIOWARN_TIME
                        ...
                        //ELSE USER IS IN "NOT" IN TABLE, SO LETS WATCH THEM
	...
	mysql_query("DELETE FROM users WHERE status = 'pending' AND added <
        FROM_UNIXTIME($deadtime)
