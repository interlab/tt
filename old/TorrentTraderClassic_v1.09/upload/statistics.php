...
begin_frame("STATISTICS");
		...
		$SITEURL2 = "$SITEURL/statistics.php";
?>
...
if(!isset($_GET['act']) && !isset($_POST['act'])) {
	...
	/*	$res = @mysql_query( "SELECT UNIX_TIMESTAMP(MAX(added)) as result_maxdate,
		...
		$html = $page_title."<br /><table id=torrenttable cellpadding=3 cellspacing=1 style='border-collapse: collapse' bordercolor=#646262 width=95% border=1><tr><td colspan=3>".ucfirst($_POST['timescale']) ." ".$table
		...
		if ( mysql_num_rows($res) )
		{
			...
			foreach( $results as $pOOp => $data )
			{
    			...
    			$html .= "<tr><td width=25%>" .$date . "</td><td width=70%><img src='images/bar_left.gif' border='0' width='4' height='11' align='middle' alt=''><img src='images/bar.gif' border='0' width='$img_width' height='11' align='middle' alt=''><img src='images/bar_right.gif' border='0' width='4' height='11' align='middle' alt=''></td><td align=right width=5%>".
	...
	function main_screen($mode='reg')
	{
		...
		$page_detail = "Please define the date ranges and other options below.<br>Note: The statistics generated are based on the information currently held in the database, they do not take into account pruned forums or delete posts, etc.<br>";
