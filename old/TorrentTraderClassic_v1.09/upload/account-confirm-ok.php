...
if ($type == "signup" && $email !== '') {
    ...
    if (! $ACONFIRM) {
        print($txt['ACCOUNT_CONFIRM_SENT_TO_ADDY'] . " (" . h($email) . "). " . $txt['ACCOUNT_CONFIRM_SENT_TO_ADDY_REST'] . " <br>");
    } else {
        print($txt['ACCOUNT_CONFIRM_SENT_TO_ADDY'] . " (" . h($email) . "). An admin needs to approve your account before you can use it <br>");
    }
	end_frame();
}
