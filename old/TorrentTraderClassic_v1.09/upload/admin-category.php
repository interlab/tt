...
$success = false;
$_GET['sort_index'] = $_GET['sort_index'] ?? '';
$_GET['delid'] = (int) ($_GET['delid'] ?? 0);

// DELETE CAT
$sure = strip_tags($_GET['sure'] ?? '');
if ($sure == "yes") {
$delid = $_GET['delid'];
...
if ($delid > 0) {
    echo '
        <br><center>Are you sure you would like to delete this category? <br>($name)<BR><br> '.
        '( <strong><a href="admin-category.php?delid='.$delid.'&cat='.$name.'&sure=yes">Y</a>'.
        '</strong> / <strong><a href="admin-category.php">N</a></strong> )<br><BR><i>Please note: '.
...
$sort_index = $_GET['sort_index'];
    ...
    $sql = DB::update('categories', ['sort_index' => $sort_index, 'name' => $cat_name, 'image' => $cat_img], ['id' => $id]);
    if ($sql) {
        echo("<CENTER><table cellspacing=0 cellpadding=5 width=50%>
        <tr><td><div align='center'><br><br>Your category has been edited <strong>succesfully!</strong><br> ".
        "[<a href='admin-category.php'>Back</a>]</div></tr>
        </table><BR><BR></CENTER>");
        end_frame();
        stdfoot();
        die();
...
if ($add == 'true') {
    ...
    $sql = DB::insert('categories', ['sort_index' => $sort_index, 'name' => $cat_name, 'image' => $cat_img]);
    if ($sql) {
        $success = true;
    } else {
        $success = false;
...
$sql = DB::query('SELECT * FROM categories WHERE 1 = 1 ORDER BY sort_index ASC');
while ($row = $sql->fetch()) {
    $id = $row['id'];
    $name = $row['name'];
    ...
    $img = $row['image'];
    echo("
    <tr><td><strong>$priority</strong></td><td><strong><a href='/?cat=$id'>$name</a></strong></td>
    <td><img src='/images/categories/$img' border=0></td>
    <td><a href='admin-category.php?editid=$id&name=$name&img=$img&sort_index=$priority'>
        <div align='center'><img src='/images/edit.gif' alt=edit border=0></a></div></td>
    <td><div align='center'><a href='admin-category.php?delid=$id&cat=$name'>
        <img src='/images/delete.gif' alt=delete align='center' border=0></a></div>
    </td></tr>");
}
echo("</table></center>");
