...
if ($user['ip'] && !(get_user_class() < UC_JMODERATOR && $user['class'] >= UC_UPLOADER)) {
    ...
    if ($dom == $user['ip'] || @gethostbyname($dom) != $user['ip']) {
        ...
        $ndomparts = count($domparts);
        $domain = $ndomparts > 1 ? $domparts[$ndomparts - 2] : '';
...
<br><a href="account-history.php?id=<?= $user['id'] ?>&action=viewposts"><b><?= $txt['VIEW_POSTS'] ?></b></a>
...
if (get_user_class() >= UC_JMODERATOR && $user['invites'] > 0
    ...
    echo '<tr><td class=rowhead>Invites: </td><td align=left>', $user['invites'], '</a></td></tr>';
}
if (get_user_class() >= UC_JMODERATOR && $user['invited_by'] > 0
    || $user['id'] == $CURUSER['id'] && $user['invited_by'] > 0
) {
    $invited_by = DB::fetchAssoc('SELECT username FROM users WHERE id = {int:id} LIMIT 1',
        ['id' => (int) $user['invited_by']]
    );
    echo '
    <tr><td class=rowhead>Invited by: </td>
        <td align=left><a href="account-details.php?id=', $user['invited_by'], '">', $invited_by['username'], '</a></td>
    </tr>';
}
...
<?php
if ($torrents)
    echo '<B>' . $txt['UPLOADED_TORRENTS'] . ':</B><BR>', $torrents, '<BR><BR>';
if ($seeding)
    echo '<B>' . $txt['CURRENTLY_SEEDING'] . ':</B><BR>', $seeding, '<BR><BR>';
if ($leeching)
    echo '<B>' . $txt['CURRENTLY_LEECHING'] . ':</B><br>', $leeching, '<BR><BR>';
end_frame();
...
<tr><td>US$&nbsp;Donated</td>
    <td align=left><input type="text" size="4" name="donated" value="', $user['donated'], '"></td></tr>
<tr><td>Password</td>
    <td align=left><input type="password" size="60" name="password" value="'. $user['password'], '"></td></tr>
<tr><td>Change Password:</td>
    <td align=left><input type="checkbox" name="chgpasswd" value="yes"></td></tr>
<tr><td>Mod Comment</td>
    <td align=left><textarea cols="60" rows="8" name="modcomment">', $modcomment, '</textarea></td></tr>
<tr><td>Account:</td>
    <td align=left><input name="enabled" value="yes" type="radio"', ($enabled ? ' checked' : ''), 
'>Enabled <input name="enabled" value="no" type="radio"', (!$enabled ? ' checked' : ''), '>Disabled</td></tr>
<tr><td>Warned: </td>
    <td align=left><input name=warned value=yes type="radio"', ($warned ? ' checked' : ''), 
'>Yes <input name=warned value=no type="radio"', (!$warned ? ' checked' : ''), '>No</td></tr>
<tr><td>Forum Banned: </td>
    <td align=left><input name=forumbanned value=yes type="radio"', ($forumbanned ? " checked" : ''),
'>Yes <input name=forumbanned value=no type="radio"', (!$forumbanned ? " checked" : ''), '>No</td></tr>';

?>
    <tr><td colspan=2><input type="submit" class="btn" value="Okay"></td></tr>
</table>
</form>

<BR>
<div align="center"><a href="admin.php?act=deluser&id=<?= $user['id'] ?>">DELETE ACCOUNT</a>
<BR>(There will be <b>NO</b> further confirmation)</div>

<?php end_frame(); ?>

<br><br>

<?php begin_frame('IP Ban', 'center'); ?>
