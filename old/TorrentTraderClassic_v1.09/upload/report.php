...
if (!empty($takeuser)) {
    ...
    $res = DB::fetchColumn("SELECT id FROM reports WHERE addedby = $CURUSER[id] AND votedfor = $takeuser AND type = 'user'");
    if (! $res) {
...
if ($user != '') {
    ...
    if ($arr["class"] >= JMODERATOR) {
        ...
        print("<b>Reason</b> (required): <form method=post action=report.php>
...
if ($torrent != '') {
    ...
    print("<h2>Are you sure you would like to report torrent <a href=details.php?id=$torrent><b>$arr[name]</b></a>?</h2>
        <p></p>
